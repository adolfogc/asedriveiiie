...
static int _2BUSReadData(reader* globalData, uchar socket, 
    ...
    uchar cmdCode = 0, mode = globalData->cards[socket].memCard.memType;
    uchar thisRead;
...
static int _2BUSWriteData(reader* globalData, uchar socket, unsigned short address,
    ...
    uchar mode = globalData->cards[socket].memCard.memType;
...
static int SendMemoryCommand (reader* globalData, uchar socket, 
    ...
    char cardType = globalData->cards[socket].memCard.protocolType;
    int retVal = ASE_OK;
            ...
            switch (cardType) {
                ...
                case PROTOCOL_MEM_3BUS:
                    if (inBuf[5] == 0x3F && inBuf[6] == 0x00)
                        globalData->cards[socket].memCard.memType = MEM_CARD_MAIN_MEM_MODE;
                    else if (inBuf[5] == 0x3F && inBuf[6] == 0x01)
                        globalData->cards[socket].memCard.memType = MEM_CARD_PROT_MEM_MODE;
                    else
